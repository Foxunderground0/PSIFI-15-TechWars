...
inline void handleKey(ESP8266WebServer & server, const int& buzzer_pin) {
  ...
  if (story_scene == 6) {
    if (command == String(login) + String(password)) {
  ...
  } else if (story_scene == 10) {
    if (command == String("ISTHISWHATYOUARELOOKINGFOR??ISTHISTHEKEYTHATYOUOHSODESIRE?")) {
      response = "Correct Key Entered";
      //markKeyAsUsed(key);
      //readNextUnenteredKey();
...
inline void handleQR(ESP8266WebServer &server) {
  ...
  server.sendHeader("Content-Disposition", "attachment; filename=qrcode.png");
